function drawGrid(dots,sDots) {
  ...
  drawSdots(dots, sDots, cnv, gridOptions.majorLines);
...
function drawSdots(dots, sDots, cnv, lineOptions){
    ...
    for (var i = 0; i < sDots.length; i++) {
        if(sDots[i].value != -1){
            ctx.fillRect(sDots[i].x -3, sDots[i].y -3, 8, 8);
        }
    }
}
